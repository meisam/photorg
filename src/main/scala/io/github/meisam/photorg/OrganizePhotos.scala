...
/**
 * The imparative version of the Photorg.
 * It uses adb (Android Debug Bridge) to pull the files from the device.
 * Don't use this file. It was just to sketching ideas on how we may interact with a device.
 */ 

/**
 * a pattern to match the lines in the output of the `adb devices` command.
 * Example intput:
 *    123456AB789CD  device
 */
...
def OrganizePhotos =
    ...
    val deviceIds = listeeDevicesRawOutput
        .collect {
            case deviceIdPattern(deviceId) => deviceId
        }
